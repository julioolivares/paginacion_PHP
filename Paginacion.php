 				...
 				elseif($pagina > $this->numero_paginas): //mostrar error 404
 					...
 					for($inicio_grupo; $inicio_grupo <= $final_grupo; $inicio_grupo++):
 						if($inicio_grupo > $this->numero_paginas): //SI EL LINK A IMPRIMIR ES > QUE EL TOTAL DE PAGINAS DETENER LA PAGINACION 
		 					...
		 					if((!isset($this->grupo_siguiente) && $this->enlace_grupo < $this->ultimo_grupo && $inicio_grupo == $final_grupo) || (($this->mostrar_pagina_ultima_no_cont === TRUE) && !isset($this->grupo_siguiente) && $inicio_grupo == $this->numero_paginas))://EVALUAR QUE NO SE HAYA CREADO EL ENLACE GRUPO SIGUIENTE Y ULTIMA PAGINA SOLO SE DEBRE CREAR UNA SOLA VEZ
		 						$link_ultima_pagina		  = $this->url_paginada.'/'.$this->numero_paginas.'/'.rand(0,9999).$this->parametros_url;
		 						$class_ultima_pagina      = '';
